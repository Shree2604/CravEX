...
#----------EXAMPLE VENDOR DATA FOR REFERENCE----------
vendor_menu={
    "Dish number 1":100,
    "Dish number 2":80
}

#for now taking 50 as an example later can be modified
vendor_stock={
    "Dish number 1":50,
    "Dish number 2":50
}
vendor_orders={}        #order_no. --> order details (This will be stored in data base later)
...
def view_vendor_orders():
    if not vendor_orders:
        ...
        print("----INCOMING ORDERS(First come first service basis)------")
...
def update_order_status():
    ...
    if order_id in vendor_orders:
        ...
        #Notification sent to users phoe 
...
def topup_stock():
    ...
    if now-last_topup_time<timedelta(hours=1):
        ...
        vendor_dashboard()
        return
    print("Current Stock :",vendor_stock)
    dish=input("Enter Dish name: ")
    qty=int(input("Enter Quantity to Add: "))

    if dish in vendor_stock:
        vendor_stock[dish]+=qty
        last_topup_time=now
        print("Successfully Updated")
    else:
        print("Invaid dish number")
